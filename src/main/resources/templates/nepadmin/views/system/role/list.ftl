<div class="layui-fluid" id="familyList" lay-title="角色管理">
...
<script>
    layui.use(['admin', 'table', 'form', 'dropdown', 'jquery'], function (admin, table, form, dropdown, $) {
        ...
        var tableIns = table.render({
            ...
            url: '${basePath}system/role/listData',
            height: 'full-165',
            cols: [[
        ...
        table.on('tool(list-table)', function (obj) {
            ...
            if (layEvent === 'edit') {
                admin.modal.open('修改角色信息', 'system/role/form?id='+data.id, {
            ...
            if (layEvent === 'view') {
                admin.modal.open('查看角色信息', 'system/role/view?id='+data.id, {
                    area: $(window).width() <= 750 ? '90%' : '50%',
                    btn: ['关闭'],
                    yes: function (index, layero) {
                        layer.closeAll();
            ...
            if (layEvent === 'del') {
                admin.modal.confirm('删除角色', '确定删除该角色吗？', function () {
                    admin.get('${basePath}system/role/delete?id='+data.id, null, function () {
                        admin.alert.success('删除角色成功');
                        query.click();
                    });
                });
            }
        ...
        add.on('click', function () {
            admin.modal.open('角色管理', 'system/role/form', {
                area: $(window).width() <= 750 ? '90%' : '50%',
                btn: ['提交', '取消'],
                yes: function (index, layero) {
                    $('#form').find('#submit').trigger('click');
